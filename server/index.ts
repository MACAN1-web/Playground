...
import { maskSnils, normalizeSnils, pool, query, queryOne, transaction } from "./db.js";
...
if (process.env.NODE_ENV === "production" && (!process.env.ADMIN_PASSWORD || !process.env.AUTH_SECRET || !process.env.APP_ORIGIN)) {
  throw new Error("Для production необходимо задать ADMIN_PASSWORD, AUTH_SECRET и APP_ORIGIN");
}

if (!process.env.ADMIN_PASSWORD) {
  throw new Error("Не задан ADMIN_PASSWORD");
}
...
const secureEquals = (first: string, second: string) => {
...
const normalizeStudyForm = (value: unknown): "Очная" | "Заочная" => {
  const normalized = String(value ?? "").trim().toLowerCase();
  return normalized.includes("заоч") ? "Заочная" : "Очная";
};
...
app.post("/api/admin/login", rateLimit("login", 8, 15 * 60_000), (request, response) => {
  const password = String(request.body.password ?? "");
  if (!secureEquals(password, process.env.ADMIN_PASSWORD || "")) {
    return response.status(401).json({ error: "Неверный пароль" });
  }
  response.json({ token: createToken() });
});

app.get("/api/admin/applicants", requireAuth, async (request, response) => {
...
app.get("/api/admin/directions/:id/applicants", requireAuth, async (request, response) => {
  const direction = await queryOne("SELECT id, specialty, study_form, budget_places, paid_places, updated_at FROM directions WHERE id = $1", [request.params.id]);
...
app.patch("/api/admin/applicants/:snils/original", rateLimit("admin-write", 240, 60_000), requireAuth, async (request, response) => {
...
app.patch("/api/admin/applicants/:snils/priority", rateLimit("admin-write", 240, 60_000), requireAuth, async (request, response) => {
  const snils = normalizeSnils(request.params.snils);
  if (snils.length !== 11) return response.status(400).json({ error: "Некорректный СНИЛС" });
  const directionId = Number(request.body.directionId);
  if (!Number.isInteger(directionId)) return response.status(400).json({ error: "Некорректная специальность" });
...
app.post("/api/admin/directions/:id/places", rateLimit("admin-write", 240, 60_000), requireAuth, updateDirectionPlaces);
app.patch("/api/admin/directions/:id/places", rateLimit("admin-write", 240, 60_000), requireAuth, updateDirectionPlaces);

app.get("/api/admin/directions/:id/export-originals", rateLimit("admin-export", 60, 60_000), requireAuth, async (request, response) => {
  ...
  worksheet.eachRow((row, rowNumber) => {
    ...
    row.eachCell({ includeEmpty: true }, (cell, columnNumber) => {
      ...
      cell.border = border;
      if (rowNumber >= 4 && budgetPlaces > 0 && rowNumber - 3 > budgetPlaces) {
...
app.post("/api/admin/import-workbook", rateLimit("admin-import", 20, 15 * 60_000), requireAuth, upload.single("file"), async (request, response) => {
...
app.delete("/api/admin/directions", rateLimit("admin-write", 240, 60_000), requireAuth, async (_request, response) => {
