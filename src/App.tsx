...
function Admin({ directions, refresh }: { directions: Direction[]; refresh: () => void }) {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      void Promise.all(changedDirections.map(({ item, changedFields }) => {
        ...
        return api.post(`/api/admin/directions/${item.id}/places`, {
          budgetPlaces: placesPayload(draft.budgetPlaces),
          paidPlaces: placesPayload(draft.paidPlaces)
        }, token).then(() => ({ id: item.id, changedFields }));
      }))
        .then((savedItems) => {
  ...
  async function login(event: React.FormEvent) {
    event.preventDefault();
    try {
      const data = await api.post<{ token: string }>("/api/admin/login", { password });
      localStorage.setItem("admin-token", data.token); setToken(data.token); setMessage("");
    } catch (error) { setMessage((error as Error).message); }
  }
  ...
  async function importWorkbook(file: File | undefined) {
    ...
    try {
      const data = await api.upload<{ importedSheets: number; importedApplicants: number; skippedRows: number; mergedDuplicates: number }>("/api/admin/import-workbook", body, token);
  ...
  async function removeAllDirections() {
    ...
    try {
      await api.delete("/api/admin/directions", {}, token);
  ...
  async function exportOriginals(directionId: number) {
    try {
      const { blob, filename } = await api.download(`/api/admin/directions/${directionId}/export-originals`, token);
  ...
  async function searchAdminApplicants(event?: React.FormEvent) {
    event?.preventDefault();
    try {
      const data = await api.get<AdminApplicant[]>(`/api/admin/applicants?q=${encodeURIComponent(adminQuery)}`, token);
  ...
  async function setOriginal(directionId: number, snils: string, originalProvided: boolean) {
    try {
      await api.patch(`/api/admin/applicants/${snils}/original`, { directionId, originalProvided }, token);
  ...
  async function setPriorityEnrollment(directionId: number, snils: string, priorityEnrollment: boolean) {
    try {
      await api.patch(`/api/admin/applicants/${snils}/priority`, { directionId, priorityEnrollment }, token);
  ...
  async function loadDirectionApplicants(directionId: number, forceRefresh = false) {
    ...
    try {
      const data = await api.get<{ applicants: AdminDirectionApplicant[] }>(`/api/admin/directions/${directionId}/applicants`, token);
      setExpandedDirectionId(directionId);
      setDirectionApplicants(data.applicants);
    } catch (error) { setMessage((error as Error).message); }
  }

  if (!token) return <section className="admin-login"><form className="panel" onSubmit={login}><p className="eyebrow">Для сотрудников</p><h2>Вход в админку</h2><input type="password" placeholder="Общий пароль" value={password} onChange={(e) => setPassword(e.target.value)} /><button>Войти</button>{message && <p className="message">{message}</p>}</form></section>;
...
export default function App() {
  ...
  useEffect(() => {
    if (!isAdminPage) localStorage.removeItem("admin-token");
  }, [isAdminPage]);

  return <><header><div className="brand"><div className="logo">К</div><div><strong>{isAdminPage ? "Панель управления" : "Приёмная комиссия"}</strong><span>{isAdminPage ? "Для работников колледжа" : "Рейтинги абитуриентов"}</span></div></div><nav>
    {isAdminPage ? <a className="nav-link" href="/">Вернуться на сайт</a> : <>
  ...
  </nav></header><main>{loadError && <p className="error-banner">{loadError}</p>}{isAdminPage ? <Admin directions={directions} refresh={refresh} /> : tab === "search" ? <SearchTab /> : <ListsTab directions={directions} />}</main></>;
}
